...
void sauvfac()
{
  GtkWidget *dialog;
  gchar *filename;
  /* char nf[40]; *///,csys[80];  avec ze = 256 cars
  gchar *nf = NULL;
  char typ[]="-F\0";
  int cbi;
  gchar *ze = NULL; /* Évite un dépassement mémoire */
  ...
  /* Libération de nf devenu inutile */
  g_free (nf); nf = NULL; /* on affecte NULL en cas d'utilisation ultérieure de nf dans cette fct */
  ...
  if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_ACCEPT)  {
...
#ifdef WIN32
    /* La longueur de filename étant indéterminé il y a un risque de dépassement
     * du tampon ze si il est déclaré en tant que tableau de taille fixe.
     * L'utilisation de g_strdup_printf(); permet d'allouer ze dynamiquement. Sa
     * taille s'adapte en fonction de la ongeur de la chaine finale.
     * La seule chose à ne pas oublier est de libérer ze une fois son utilisation
     * devenue obsolète.
     */
    ze = g_strdup_printf ("copy factest.pdf %s",filename);
    /* sprintf(ze,"copy factest.pdf %s",filename); */
#else
     /* La longueur de filename étant indéterminé il y a un risque de dépassement
     * du tampon ze si il est déclaré en tant que tableau de taille fixe.
     * L'utilisation de g_strdup_printf(); permet d'allouer ze dynamiquement. Sa
     ...
     * devenue obsolète.
     */
    ze = g_strdup_printf ("cp factest.pdf %s",filename);
    /* sprintf(ze,"cp factest.pdf %s",filename); */
# endif
    g_free (filename);
  }
  ...
  /* Libération de ze devenu inutile */
  g_free (ze);
}
...
int paramlf(int nc)
{
  ...
  if ( (k = cherpara( "pol=")) != -1)	{
    ++k; parlf[nc].nupol=0;
    for (i=0; i < nbpol; ++i)	{
      if ( strcmp(tabmot[k].pmot,tabpol[i].pol) == 0) { parlf[nc].nupol= i; break;}
    } 
  }  
...
int param0()
{
  ...
  if ( (k = cherpara( "siz=")) != -1) par0.siz = g_strtod(tabmot[++k].pmot,NULL);
  else par0.siz =6;
  if ( (k = cherpara( "pol=")) != -1)	{
    ++k; par0.nupol=0;
    for (i=0; i < nbpol; ++i)	{
...
void implf()
{
  ...
  for ( lf=0; lf < derlig; ++lf)	{
	...
	gchar *zn = g_strdup_printf ("%10s",zn2);
	/* sprintf(zn,"%10s",zn2); */
